...
export class SlaTemplatesCreateComponent implements OnInit {
	...
	ngOnInit() {
		this.section = this.route.url['value'][0].path.replace(/-/g, ' ').toUpperCase();
		...
		Promise.all([this.commonService.getNetworkServices(this.section), this.servicePlatformService.getServiceGuarantees()])
			...
			.catch(err => {
				this.loading = false;
				let title = 'oh oh...';
				let content = 'Unable to create any template without network services and guarantees!';
				let action = 'Accept';
		...
		this.servicePlatformService
			...
			.catch(err => {
				...
				if (err.error['ERROR: '] === 'Conflict') {
					...
					let title = 'oh oh...';
					let content = 'The expire date is not a future date. Please choose another.';
					let action = 'Accept';
